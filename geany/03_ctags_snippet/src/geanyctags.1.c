...
static GtkWidget
    *s_context_fdec_item, //Find Tag Declaration (geanyctags)コンテキストメニュー
    *s_context_fdef_item,   //Find Tag Definition (geanyctags)コンテキストメニュー
    *s_context_sep_item,    //コンテキストメニュー セパレータ
	*s_gt_item, //メニュー プロジェクト Generate tags
    *s_sep_item,    //メニュー プロジェクト Vセパレータ
    *s_ft_item; //メニュー プロジェクト Find tags
...
/*
    TODO:一部のAPIがプライベートであるため、以下の方法で行うことはできません Geanyで
    これは、カーソルを最初にシンボルに置く必要があることを意味します。
    に右クリック(シンボルにカーソルを置かずに右クリック) 動作しません)
*/
...
//コンテキストメニューからのfind declaration CALLBACK
...
//コンテキストメニューからのfind definition CALLBACK
